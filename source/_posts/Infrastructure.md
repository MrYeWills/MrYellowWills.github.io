...
而其他的错误码，包含 未登录 等的处理，就交给 比如pc端的脚手架_wo_rk处理了。
...
-3304006 无权限 直接 跳转到 无权限页面 /noauthority ；
-13305002, "系统需要钉钉扫码登录进行访问"  直接执行一次 /authcenter/exitUrl 登出操作 并提示；
-13305003, "帐号已在其他地方登录，当前系统被迫下线"  跳转登录页面；
等等；
...
#### 根据ac-session-id 检测
判断登录与否是根据 ac-session-id ，而不是我们通常认为的 account 账号信息，这也是值得注意或借鉴的。
...
所有的 iframe 嵌入页面 公用一个 路由： '/iframe?url=http://www.baidu.com'
...
## 存在的问题
优化：
目前无权限页面，会首先显示一下真实的页面内容，然后闪一下，跳转到无权限页面，
可以优化成，不用显示真实页面内容，直接跳转到无权限页面；
