...
此灵感来自于babel编译底层对 函数作用域的编译原理， 在babel编译js时，也会生成一个函数作用域的scope对象，原理就是基于 var 等能够声明的特殊关键字 收集所有的作用域变量，最终形成一个作用域object对象。
...
为什么很多人对闭包非常理解非常困惑，其本质是不懂 执行上下文销毁的机制是什么，销毁的判定标准是什么。
